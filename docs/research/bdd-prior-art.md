...
updated: 2026-08-29
status: draft
confidence: findings come from web-search result summaries, not direct page fetches (WebFetch was unavailable this pass); treat quotes as search-engine-paraphrased unless marked direct; the open-source-adoption statistic below is secondary-sourced and unverified
---

> **Method note:** this is a search-summary pass, not a fetched-and-read pass. No claim here should be treated as a direct quote unless stated. One statistic is flagged `⚠️ unverified` — do not cite it anywhere that matters without checking the primary study.
...
The newest LLM research reinforces this from the opposite direction. Generated specs already hit ~94% semantic coverage against source requirements — but the residual failure mode is **omission that still passes**: in a food-safety-law case study, LLM-generated Gherkin from 10 participants showed omissions and hallucinated content as the dominant failure, and an overlooked constraint "can pass testing" while being unsafe — the generated test would be green and wrong ([arxiv.org/abs/2508.20744](https://arxiv.org/abs/2508.20744)). Automating the glue doesn't remove the risk; it moves it from "stale step definition" to "silently incomplete generated spec."
...
- *"From Law to Gherkin"* ([arXiv 2508.20744](https://arxiv.org/abs/2508.20744)) — human-subject study, 10 participants, 60 LLM-generated (Claude + Llama) Gherkin specs from food-safety law. Finding: usable as a first draft, but omissions and hallucinated content are the dominant failure mode; in safety-critical domains an overlooked constraint "can pass testing" while being unsafe.
