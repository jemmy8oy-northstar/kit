# Repo setup — inherited from `repo-template`

_This is the README `kit` was created with, kept here rather than deleted: the root README is now the
project's, but the workflow / ruleset / secrets notes below are still the operating instructions for
this repo. **Nothing in it has been acted on yet** — in particular the two rulesets are **not applied**
(`gh api repos/jemmy8oy-northstar/kit/rulesets` returns `[]`), so `main` and `dev` are currently
unprotected. Applying them is James's, per the "never yours" list in `claude-code-bot`'s CLAUDE.md._
